...
plot(test_data$timestamp, test_data$value, type = "l", col = "blue", lwd = 2, ylim = range(c(y_pred, test_data$value)), xlab = "date", ylab = "migration count", main = "AR(1) Model Predictions")
lines(test_data$timestamp, y_pred, col = "red", lwd = 2, lty = 2)
legend("topleft", legend = c("Actual Migration", "AR(1) Predicted Migration"), col = c("blue", "red"), lty = c(1, 2), bty = "n")

```
```{r}
ar_model <- ar(y_train, order.max = 29, aic = FALSE, method = "ols", demean = FALSE)  # AR(1)
# get the parameter
ar_model$ar
# To estimate an intercept (the mean of the series), set demean = TRUE (the default)
# ar_model <- ar(y_train, order.max = 1, aic = FALSE, method = "ols", demean = TRUE)  # AR(1)
# get the parameters
# c(ar_model$ar, ar_model$x.intercept)

# To check the roots, need to manually calculate
# Don't worry too much about this math
# Once again, these should be > 1 to meet the stationarity assumption
...
pred_data <- data.frame(Year = test_data$timestamp, pred_SO9 = ar_pred$pred)

# Plot actual vs predicted from AR model
plot(test_data$timestamp, test_data$value, type = "l", col = "blue", lwd = 2, xlab = "Year", ylab = "Strikeout Rate", main = "AR(1) Model Predictions")
lines(pred_data$Year, pred_data$pred_SO9, col = "red", lwd = 2, lty = 2)
legend("topleft", legend = c("Actual SO9", "AR(1) Predicted SO9"), col = c("blue", "red"), lty = c(1, 2), bty = "n")
