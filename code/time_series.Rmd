...
y_train_ts <- ts(y_train, frequency=12)

sarima_model <- auto.arima(y_train_ts[,"value"], seasonal = TRUE, stepwise = FALSE, approximation = FALSE)

forecast_steps <- length(y_test)

# Generate forecasts
sarima_forecast <- forecast(sarima_model, h = forecast_steps)

# View predicted values
predicted_values <- sarima_forecast$mean
```
```{r}
# Plot forecast
plot(sarima_forecast, main = "SARIMA Forecast vs Actual")

# Add actual y_test values to the plot
lines(ts(y_test, start = end(time(y_train_ts)) + c(0,1), frequency = 12), col = "red")
